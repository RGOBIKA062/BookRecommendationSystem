...
const LibraryManager = ({ 
  ...
  // Smart suggestions based on book genre/category
  const getSmartSuggestions = () => {
    ...
    if (title.includes('finance') || title.includes('money') || title.includes('budget')) {
      suggestions.push('� Financial Planning', '� Money Management', '📈 Investment Reads');
    } else if (title.includes('british') || author.includes('british')) {
      suggestions.push('🇬🇧 British Literature', '📚 UK Authors', '🏴󠁧󠁢󠁥󠁮󠁧󠁿 British Culture');
    } else if (title.includes('kids') || title.includes('children')) {
      suggestions.push('👶 Children\'s Books', '📖 Family Reading', '🌟 Educational Stories');
    } else {
      // General suggestions for new users
      suggestions.push('📚 Want to Read', '⭐ Favorites', '🎯 Currently Reading');
    }
  ...
  const handleSuggestionClick = (suggestion) => {
    // Clean suggestion text for library name
    const cleanName = suggestion.replace(/📊|💰|📈|🇬🇧|📚|🏴󠁧󠁢󠁥󠁮󠁧󠁿|👶|📖|🌟|⭐|🎯|✅/g, '').trim();
    setNewLibraryName(cleanName);
  };
          ...
          <div className="welcome-card mb-4 p-4 bg-gradient-primary text-white rounded-3">
            <div className="text-center">
              ...
              <p className="mb-0 opacity-90">
                Let's create your first library to organize your books perfectly.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="create-library-tab">
            ...
            <div className="mb-3">
              <small className="text-muted fw-medium d-block mb-2">
                {isNewUser ? '🌟 Perfect starter libraries for you:' : '💡 Smart Suggestions'}
              </small>
              <div className="d-flex flex-wrap gap-2">
                {getSmartSuggestions().map((suggestion, index) => (
                  <Button
                    ...
                    className="rounded-pill border-2 suggestion-btn"
                  >
                    {suggestion}
                  </Button>
                ))}
            ...
            <Form.Group className="mb-4">
              ...
              <Form.Control
                type="text"
                placeholder={isNewUser ? "e.g., My Reading List, Favorites, Want to Read..." : "e.g., Fiction Favorites, Technical Books, Want to Read..."}
